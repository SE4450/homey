      ...
      <View style={styles.tableContainer}>
        ...
        {listItems.map((item, index) => (
          <View key={"viewnode_row" + index} style={styles.tableBody}>
            <View
              key={item.item + "purchaseButtonnode"}
              style={styles.tableRow}
            >
              <Pressable
                key={item.item + "purchaseButton_"}
                onPress={() => {
                  confirmPurchase(item.itemId, item.item, item.purchased);
                }}
              >
                <Text style={styles.button}>Confirm Purchase</Text>
              </Pressable>
            </View>
            <View key={item.item + "viewnode"} style={styles.tableRow}>
              <Text key={item.item + "textnode"} style={styles.tableData}>
                {item.item}
              </Text>
            </View>
            <View
              key={item.item + "AssignedToviewnode"}
              style={styles.tableRow}
            >
              <Dropdown
                options={["Roomate1", "Roomate2"]}
                onOptionSelect={(option) =>
                  updateAssigment(item.itemId, option)
                }
                customStyle={{
            ...
            <View key={item.item + "purchasedviewnode"} style={styles.tableRow}>
              <Text
                key={item.item + "purchasedtextnode"}
            ...
            <Pressable
              ...
              onPress={() => {
                deleteRow(item.itemId);
              }}
            >
              <Text style={styles.button}>Delete Row</Text>
            </Pressable>
          </View>
        ))}
      </View>
