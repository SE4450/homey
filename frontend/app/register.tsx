...
const RegisterScreen = () => {
    ...
    const validatePassword = (password: string): { isValid: boolean; failedRules: any[] } => {
    ...
    const handleRegister = async () => {
        ...
        if (!password || !passwordValidation.isValid) {
            const failures = passwordValidation.failedRules.reduce((current, rule, index) => current + `${index + 1}. ${rule.message.replace("string", "password")}\n`, "");
